...
def depth_first(graph_map, start_node, seen, order):
    order.append(start_node)
    seen[start_node] = True
    for adjacent in graph_map[start_node]:
        if seen[adjacent] == False:
            depth_first(graph_map, adjacent, seen, order)
            seen[adjacent] = True
    return order
...
for _ in range(num_edges):
    ...
    graph_map[node2].append(node1)

# print(graph_map)
start_node = 'A'
seen = defaultdict(bool)
order = []
dfs_path = depth_first(graph_map, start_node, seen, order)
print(dfs_path)
```
